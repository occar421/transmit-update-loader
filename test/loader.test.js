...
test('does not affect file content', async () => {
    ...
    const status = await compiler(targetFileName, {transmitRules: [{test: /\/(.*$)/, targets: ['$1.null']}]});
    const output = status.toJson().modules[0].source;
    expect(output).toBe(await readFileAsync(targetFileName));
});

test('does not renew itself', async () => { // to prevent infinite renewing
    const targetFileName = './fixtures/foo.txt';
    const preStat = await fileStatAsync(targetFileName);
    await compiler(targetFileName, {transmitRules: [{test: /\/(.*$)/, targets: ['$1']}]}); // identity mapping
    const postStat = await fileStatAsync(targetFileName);
    expect(postStat.ctimeMs).toBe(preStat.ctimeMs);
});

test('does not renew itself within multi targets', async () => { // to prevent infinite renewing
    const targetFileName = './fixtures/foo.txt';
    const preStat = await fileStatAsync(targetFileName);
    await compiler(targetFileName, {transmitRules: [{test: /\/(.*$)/, targets: ['$1', '$1', 'foo', 'bar']}]});
    const postStat = await fileStatAsync(targetFileName);
    expect(postStat.ctimeMs).toBe(preStat.ctimeMs);
});

test('does not renew itself within multi rules', async () => { // to prevent infinite renewing
    const targetFileName = './fixtures/foo.txt';
    const preStat = await fileStatAsync(targetFileName);
